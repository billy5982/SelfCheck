...
const List = ({value,idx,inputData,setInputData}) =>{
  ...
  const saveNewdata = (idx) =>{
     const copyData = [...inputData]; // 원본데이터를 직접 수정할 수 없어서 데이터를 복사해야함
...
const Input = ({inputData,setInputData}) => {
  ...
  const saveData = () =>{
    if(!inputData.includes(inputValue)){ //중복되는 것은 추가되지 않음
      setInputData([inputValue,...inputData]) // 원본 배열에 데이터를 추가
      setInputValue('');
      ref.current.focus()
  ...
  return(
    <div>
      ...
      <button onClick={dataSave}>저장 </button>
    </div>
  )
}
...
function App() {
  ...
  return(
    <div>
      ...
      <ul>
      {inputData.length!==0 ? (inputData.map((el,i)=>
      (<List key = {`i+${i}`} value = {el} idx = {i} inputData = {inputData} setInputData={setInputData}/>))) : null
      }
      </ul>
    </div>
  );
